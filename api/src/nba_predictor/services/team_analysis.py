...
class TeamAnalysisService:
    ...
    def get_recent_form(self, team_id: int, num_games: int = 10) -> Record:
    ...
    def get_recent_games(self, team_id: int, num_games: int = 10) -> list[dict]:
        """Get details of last N games."""
        games = self.get_team_games(team_id)
        recent = games[-num_games:] if len(games) >= num_games else games
    ...
    def get_all_trends(self, team_id: int, season: str | None = None) -> list[TrendInsight]:
        ...
        recent = self.get_recent_form(team_id, num_games=10)
        if recent.total >= 5:
            trends.append(TrendInsight(
    ...
    def get_game_insights(
        ...
        # Get last game results to determine if coming off win/loss
        home_games = self.get_team_games(home_team_id)
        away_games = self.get_team_games(away_team_id)
        ...
        # Calculate rest days (would need today's date and last game date)
        home_rest = None
        away_rest = None
        if home_games:
        ...
        return {
            "home_team": {
                ...
                "recent_games": self.get_recent_games(home_team_id, 5),
            },
            "away_team": {
                "id": away_team_id,
                ...
                "recent_games": self.get_recent_games(away_team_id, 5),
            },
            "head_to_head": {
